 ...
 */
 #define HOR_LEN 	30	//	Alse mind the resolution of your screen!
uint8_t disp_buf[2*ST7789_WIDTH * HOR_LEN];
#endif



//extern uint8_t dma_spi1_fl = 0;
//extern uint32_t dma_spi1_count = 1;
//uint8_t dma_buffer[7200] = 0;
...
void ST7789_Init(void)
{
	...
	HAL_Delay(50);
	//ST7789_Fill_Color(0xFF00);				//	Fill with Black.
}
...
void ST7789_Fill_Color(uint16_t color)
{
	...
	#ifdef USE_DMA
		for (i = 0; i < ST7789_WIDTH * HOR_LEN; i++)
		{
			disp_buf[2*i] = color >> 8;
			disp_buf[2*i + 1] = color & 0xFF;
		}
 ...
 * @return none
 */
void ST7789_FillRectangle(uint16_t xSta, uint16_t ySta, uint16_t xEnd, uint16_t yEnd, uint16_t color)
{
	if (xSta >= ST7789_WIDTH) xSta = ST7789_WIDTH - 1;
	if (xEnd >= ST7789_WIDTH) xEnd = ST7789_WIDTH - 1;
	if (ySta >= ST7789_HEIGHT)ySta = ST7789_HEIGHT - 1;
	if (yEnd >= ST7789_HEIGHT) yEnd = ST7789_HEIGHT -1;
	uint16_t buff;
	if (xSta > xEnd)
	{
		buff = xSta;
	...
	ST7789_SetAddressWindow(xSta, ySta, xEnd, yEnd);
		...
		j = ((xEnd - xSta) * (yEnd - ySta))/(ST7789_HEIGHT*HOR_LEN) + 1;
		for (i = 0; i < j; i++)
		{
			ST7789_WriteData(disp_buf, sizeof(disp_buf));
		}
...
void ST7789_DrawPixel_4px(uint16_t x, uint16_t y, uint16_t color)
{
	if ((x <= 0) || (x > ST7789_WIDTH) ||
		 (y <= 0) || (y > ST7789_HEIGHT))	return;
	ST7789_Select();
	ST7789_FillRectangle(x - 1, y - 1, x + 1, y + 1, color);
	ST7789_UnSelect();
}
...
uint16_t ST7789_RandomColor(void)
{
